...
const projectLink = (p) => (p.demo ? `<a class="artifact-open" href="${escape(p.demo.url)}" target="_blank" rel="noopener noreferrer">${escape(p.demo.label)} ${externalIcon}<span class="sr-only"> (opens in a new tab)</span></a>` : `<a class="artifact-open" href="${escape(p.media.src)}" target="_blank" rel="noopener noreferrer">View full-size image ${externalIcon}<span class="sr-only"> (opens in a new tab)</span></a>`) + (p.live ? `<a class="artifact-open" href="${escape(p.live.url)}" target="_blank" rel="noopener noreferrer">${escape(p.live.label)} ${externalIcon}<span class="sr-only"> (opens in a new tab)</span></a>` : '');
const projectArtifact = (p) => p.demo ? `<div class="demo-window"><div class="demo-window-bar"><span>Interactive / fictional prototype data</span><span>Compiled HTML</span></div><iframe src="${escape(p.demo.url)}${p.demo.previewAnchor ? `#${escape(p.demo.previewAnchor)}` : ''}" title="${escape(p.title)} — interactive fictional-data prototype" loading="eager" sandbox="allow-scripts allow-downloads" referrerpolicy="no-referrer"></iframe></div>` : `<figure class="image-artifact"><a href="${escape(p.media.src)}" target="_blank" rel="noopener noreferrer" aria-label="Open full-size image for ${escape(p.title)} in a new tab"><img src="${escape(p.media.src)}" width="${p.media.width}" height="${p.media.height}" alt="${escape(p.media.alt)}" loading="lazy" decoding="async"></a><figcaption>${escape(p.media.caption)}</figcaption></figure>`;
