...
for (const project of projects) {
  ...
  if (!project.media && !project.demo) throw new Error(`Project has no artifact: ${project.id}`);
...
const projectLink = (p) => p.demo ? `<a class="artifact-open" href="${escape(p.demo.url)}" target="_blank" rel="noopener noreferrer">${escape(p.demo.label)} ${externalIcon}<span class="sr-only"> (opens in a new tab)</span></a>` : `<a class="artifact-open" href="${escape(p.media.src)}" target="_blank" rel="noopener noreferrer">View full-size image ${externalIcon}<span class="sr-only"> (opens in a new tab)</span></a>`;
const projectArtifact = (p) => p.demo ? `<div class="demo-window"><div class="demo-window-bar"><span>Interactive / fictional prototype data</span><span>Compiled HTML</span></div><iframe src="${escape(p.demo.url)}" title="${escape(p.title)} — interactive fictional-data prototype" loading="eager" sandbox="allow-scripts allow-downloads" referrerpolicy="no-referrer"></iframe></div>` : `<figure class="image-artifact"><a href="${escape(p.media.src)}" target="_blank" rel="noopener noreferrer" aria-label="Open full-size image for ${escape(p.title)} in a new tab"><img src="${escape(p.media.src)}" width="${p.media.width}" height="${p.media.height}" alt="${escape(p.media.alt)}" loading="lazy" decoding="async"></a><figcaption>${escape(p.media.caption)}</figcaption></figure>`;
const projectSection = (p, lead = false) => `<section class="project-section${lead ? ' project-section--lead' : ''}" id="${escape(p.id)}" aria-labelledby="title-${escape(p.id)}" data-project="${escape(p.id)}"><div class="project-heading"><div><h2 id="title-${escape(p.id)}">${escape(p.title)}</h2><p class="project-kind">${escape(p.kind)}</p></div>${projectLink(p)}</div><p class="project-summary">${escape(p.summary)}</p><p class="disclosure"><strong>About this artifact</strong> ${escape(p.disclosure)}</p>${projectArtifact(p)}<p class="project-context">${escape(p.context)}</p></section>`;
...
const departures = worlds.slice(1).map((world) => `<li><a href="${href(world)}"><span class="departure-name">${escape(world.name)}</span><span class="departure-note">${escape(world.description)}</span><span class="departure-status">Preview route open <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M2 10h15m-6-6 6 6-6 6"/></svg></span></a></li>`).join('');
